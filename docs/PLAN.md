...
Set up the Docker infrastructure, the backend in backend/ with Spring Boot, and write the start and stop scripts in the scripts/ directory. This should serve example static HTML to confirm that a 'hello world' example works running locally and also make an API call.
...
Now propose a database schema for the Kanban. Document the database approach in docs/ and get user sign off.

Part 6: Backend

Now add API endpoints to allow the backend to read and change the Kanban for a given user; test this thoroughly with backend unit tests. The database should be created if it doesn't exist.
